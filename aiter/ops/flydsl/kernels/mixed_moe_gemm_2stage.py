        ...
        def moe_gemm2(
            ...
            def _moe_gemm2_then_body():
                ...
                def load_b_packs_k64(base_k, ku: int, ni: int):
                    """Load one B MFMA k-step (fp4: 2x i64; fp8-B: 4x i64)."""
                    base_k_bytes = base_k * arith.constant(
                        int(b_elem_bytes), index=True
                    )
                ...
                def _accum_b_ku(b_tile, base_k, ku):
                    """Append one ku's N-packs to b_tile.

                    fp4: (packs0, packs1); fp8-B: (packs0, packs1, packs2, packs3).
                    """
                    packs0, packs1 = [], []
                    packs2, packs3 = [], []
                    for ni in range_constexpr(num_acc_n):
                        b = load_b_packs_k64(base_k, ku, ni)
                        packs0.append(b[0])
                        packs1.append(b[1])
                        if const_expr(is_f8_b):
                            packs2.append(b[2])
